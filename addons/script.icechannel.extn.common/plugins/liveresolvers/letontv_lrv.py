...
class letontv(LiveResolver):
    ...
    def ResolveLive(self, content, url):
        ...
        new_content = re.search('src=[\'"]{1}http://leton\.tv/player\.php\?streampage=(.+?)[&\'"]{1}', content)

        if new_content:
        
            page_url = 'http://leton.tv/player.php?streampage=%s' % new_content.group(1)
